...
class Page{
    ...
    public function show() {
		...
		{
		  $link_page.="<span><select onchange='location.href=this.options[this.selectedIndex].value;'>";
          for($i=1;$i<=$this->rollPage;$i++){
            if(($this->nowPage - $now_cool_page) <= 0 ){
				$page = $i;
			}elseif(($this->nowPage + $now_cool_page - 1) >= $this->totalPages){
            ...
            if($page!=$this->nowPage)
			{
               if($page<=$this->totalPages)
			   {
                  $link_page .= "<option value='".$this->url($page)."'>".$page."</option>";
               }
			   else
			   {
                  break;
			...
			else
			{
               if($this->totalPages != 1)
			   {
                  $link_page .= "<option value='".$this->url($page)."' selected='selected'>".$page."</option>";
               }
            }
          }
          $link_page.="</select></span>";
        }
        //替换分页内容
