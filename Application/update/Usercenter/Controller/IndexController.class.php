...
class IndexController extends Controller
{
    ...
    public function information($page = 1, $r = 20)
    {
			...
			$sql="select studentScore from user_student where student_userId=2854";
			$data=$this->remote->query("select studentScore from user_student where student_userId=2854");
			if(!$data)
			{
			  $data["score"]="未知";
			}			
			...
			$data=$this->remote->query("select w.rumen from user_student s , writingGrade w where s.studentId=w.studentId and w.isdelete=0 and s.student_userId=2385");
			if(!$data)
			{
			  $data["score"]="未知";
			}
