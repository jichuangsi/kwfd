...
class AdminListBuilder extends AdminBuilder
{
    ...
    {
        return $this->button('清空', array('class' => 'btn ajax-post tox-confirm', 'data-confirm' => '您确实要清空所选数据吗？（清空后不可恢复）', 'url' => U('', array('model' => $model)), 'target-form' => 'ids'));
    }
    ...
    public function keyCreateTime($name = 'createtime', $title = '创建时间')
    {
        return $this->keyTime($name, $title);
    }

    public function keyUpdateTime($name = 'updatetime', $title = '更新时间')
    {
        return $this->keyTime($name, $title);
    }
    ...
    public function keyDoAction($getUrl, $text, $title = '操作',$attr=array())
    {
        //获取默认getUrl函数
        if (is_string($getUrl)) {
            $getUrl = $this->createDefaultGetUrlFunction($getUrl);
        ...
        //在DOACTIONS中增加action
        $doActionKey['opt']['actions'][] = array('text' => $text, 'get_url' => $getUrl,'attr'=>$attr);
        return $this;
    }
	public function keyDoActionMorePara($getUrl, $text, $title = '操作')
    {
        //获取默认getUrl函数
        if (is_string($getUrl)) {
            $getUrl = $this->createGetUrlFunction($getUrl);
        }
    ...
    public function hookCombiner($postion, $data = '')
    {
        if (!empty($postion) && !empty($data)) {
            $this->assign('combiner_hook_' . $postion, $data);
        }
        return $this;
    }
    ...
    private function createGetUrlFunction($pattern)
    {
        return function ($item) use ($pattern) {
            //$pattern = str_replace('###', $item['id'], $pattern);
			preg_match_all('/[?&].+?=.+?/i' ,$pattern ,$res);
            //var_dump($res);
			...
			if(sizeof($res)>0)
			{
              foreach($res[0] as $key=>$val)
              {
				 ...
				 $val=substr($val,1);
				 $valarr=explode('=',$val);
				 $pattern.='&'.$valarr[0].'='.$item[$valarr[0]];
              }
			}
            //调用ThinkPHP中的解析引擎解析变量
            $view = new \Think\View();
