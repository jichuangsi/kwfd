...
$(function () {
    ...
    $('.ajax-post').click(function () {
        ...
        if (($(this).attr('type') == 'submit') || (target = $(this).attr('href')) || (target = $(this).attr('url'))) {
            ...
            /*剥离禁用用户 start*/
            var forbidBtn = $(this).attr('data-forbid') ;
            if(forbidBtn == 'forbid') {
                doPostForbid(form, true) ;
                return false;
            }
            /*剥离禁用用户 end*/
...
var modalOS=function(param){
    ...
    $tag.on('loaded.bs.modal',function(){
        ...
        if($(this).find('.modal-header').length==0){
            ...
            html='<button type="button" class="close" data-dismiss="modal" aria-label="Close"></button><h4 class="modal-title" id="myModalLabel">'+param.title+'</h4>';
            $(this).find('.modal-header').html(html);
        }
    });
    return this;
}