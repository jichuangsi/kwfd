...
class AboutusController extends AdminController
{
     ...
     * @param int $r
     */
	public function lists($page = 1, $r = 10)
    {
		...
		$builder->keyId()->keyText('title', '名称')->keyText('sort', '排序')->keyUpdateTime('changetime', '更新时间')->keyCreateTime('createtime', '创建时间')->keyDoActionEdit($this->_model.'/add?id=###')
    ...
    public function add($id = 0, $title = '', $image = '', $content = '', $categoryid = 0,$status = '')
    {
	   ...
	   if (IS_POST) 
	   {
			...
			if ($isEdit) {
                ...
                $lastid = $this->datamodel->order('sort DESC')->limit(1)->getField('sort');
		        $data['sort'] = $lastid+1;
