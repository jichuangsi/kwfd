...
class DownloadController extends AdminController
{
    ...
    protected $_model="Download";
	protected $_modelname="资料下载";
	protected $datamodel;
     ...
     * @param $sell_num
	 */
    public function add($id = 0, $title = '', $image = '', $file = '', $content = '', $categoryid = 0, $price=0,$status = '')
    {
	   $isEdit = $id ? 1 : 0;
	   if (IS_POST) 
	   {
	   ...
	   else 
	   {
			...
			$builder->keyId()->keyText('title', $this->_modelname.'名称')->keySelect('categoryid', '分类', '', $options)->keySingleImage('image', '图标')->keySingleFile('file', '文件')->keyEditor('content', '详情')
                ->keyText('price', '价格','')
				->keyStatus('status', '状态');
            if ($isEdit) {
