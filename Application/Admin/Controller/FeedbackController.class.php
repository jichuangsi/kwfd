...
class FeedbackController extends AdminController
{
     ...
     * @param int $r
     */
	public function lists($page = 1, $r = 5)
    {
		...
		$builder->buttonDelete(U('setStatus'))->setStatusUrl(U('setStatus'));

		$builder->keyId()->keyText('contact', '联系人')->keyText('mobile', '联系电话')->keyText('weixin','微信')->keyText('qq','QQ')->keyUpdateTime('changetime', '更新时间')->keyCreateTime('createtime', '创建时间')->keyDoActionEdit($this->_model.'/add?id=###',"查看")
    ...
    public function add($id = 0, $title = '', $image = '', $content = '', $categoryid = 0,$status = '')
    {
	   ...
	   else 
	   {
			...
			$builder->keyId()->keyText('contact', '联系人')
			->keyText('mobile', '联系电话')
			->keyText('weixin', '微信')
			->keyText('qq', 'QQ')
