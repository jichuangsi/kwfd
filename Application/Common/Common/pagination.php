...
function getPagination($totalCount, $countPerPage = 10)
{
    ...
    if($pageCount != 1)
    {
	  $html.="<li><a style='padding: 3px;'><select onchange='location.href=this.options[this.selectedIndex].value;' style='height:26px'>";
	  for ($i = 1; $i <= $pageCount; $i++) 
	  {	
		$pageUrl = addUrlParam(getCurrentUrl(), array($pageKey => $i));
        if ($i == $currentPage) {
			$html .= "<option value=\"{$pageUrl}\"  selected='selected'>{$i}</option>";
        } else {
            $html .= "<option value=\"{$pageUrl}\">{$i}</option>";
        }
	  }
	  $html.="</select></a></li>";
    }
