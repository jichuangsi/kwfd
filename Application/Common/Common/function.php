...
function is_remote_login(){
    $user = session('remote_user_auth');
    if (empty($user)) {
        return 0;
    } else {
        return session('remote_user_auth_sign') == data_auth_sign($user) ? $user['uid'] : 0;
    }
}
/**
 * 系统公共库文件
...
function msubstr($str, $start=0, $length, $charset="utf-8", $suffix=true) {
	...
	//return $suffix ? $slice.(mb_strlen($str, 'utf8') > $length?'...':'') : $slice;
}
...
{
   $defaultimage="/Public/Common/images/default.gif"; 
   if(!empty($image))
   {
     $defaultimage=$image;
   }
