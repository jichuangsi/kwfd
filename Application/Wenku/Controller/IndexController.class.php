...
class IndexController extends Controller
{
     ...
     * 初始化
     */
	protected $_model="Wenku";
    protected $_modelname="文库";
    protected $datamodel;
    ...
    public function Detail($id = 0,$category=1)
    {
		...
		$categorydata = D('WenkuCategory')->where($categorymap)->order('createtime desc')->select();
		//dump($categorydata[0]['title']);
				/* 更新浏览数 */
		$map = array('id' => $id);
		D('Wenku')->where($map)->setInc('view');
		
		$this->assign('categorytitle', $categorydata[0]['title']);
		
		        //查看最多
		$hotmap['status'] = 1;
        $hotdata = D('Wenku')->where($hotmap)->order('view desc')->limit(10)->select();
